...
(def direction (atom {:north cold-war :west welcome :south nil :east ww2}))
...
;; I want the bgvid video componenet to rerender when there is an update to :locale-bg
...
(defn available-directions [cardinal]
  (when-let [direction-info (cardinal @direction)]
    #(rf/dispatch [:screen-change direction-info])))
  ...
  [:div.w-100
   ...
   [:div.pt6.flex.items-center
    [:div {:style {:float "left"}}
     [:button {:on-click (available-directions :west)} "←"]]
    [:div {:style {:float "left"}}
     [:button {:on-click (available-directions :north)
               :style {:display "block"}} "↑"]
     [:button {:on-click (available-directions :south)
               :style {:display "block"}}"↓"]]
    [:div {:style {:float "left"}}
     [:button {:on-click (available-directions :east)} "→"]]]]])
